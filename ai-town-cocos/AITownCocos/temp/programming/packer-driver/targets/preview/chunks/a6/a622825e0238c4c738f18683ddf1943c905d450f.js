System.register(["__unresolved_0", "cc", "__unresolved_1", "__unresolved_2"], function (_export, _context) {
  "use strict";

  var _reporterNs, _cclegacy, crtptos, observer, Request, _crd;

  function _reportPossibleCrUseOfcrtptos(extras) {
    _reporterNs.report("crtptos", "../../core/network/http/cryptos", _context.meta, extras);
  }

  function _reportPossibleCrUseOfobserver(extras) {
    _reporterNs.report("observer", "../game/App", _context.meta, extras);
  }

  _export("default", void 0);

  return {
    setters: [function (_unresolved_) {
      _reporterNs = _unresolved_;
    }, function (_cc) {
      _cclegacy = _cc.cclegacy;
    }, function (_unresolved_2) {
      crtptos = _unresolved_2.default;
    }, function (_unresolved_3) {
      observer = _unresolved_3.observer;
    }],
    execute: function () {
      _crd = true;

      _cclegacy._RF.push({}, "872f1xPsHpErYpEhFUDtrSo", "Request", undefined);

      _export("default", Request = class Request {
        constructor() {}
        /**
         * 
         * @param url 请求链接
         * @param data 请求数据
         * @param type 是否是表单格式
         * @param isEencrypt 是否需要加密
         */


        static post(url, data, type, isEencrypt) {
          if (url === void 0) {
            url = '';
          }

          if (data === void 0) {
            data = null;
          }

          if (type === void 0) {
            type = {
              isForm: false
            };
          }

          if (isEencrypt === void 0) {
            isEencrypt = {
              isEencrypt: false
            };
          }

          var xhr = new XMLHttpRequest();
          xhr.withCredentials = true;
          xhr.timeout = 5000;
          console.warn("111111");
          return new Promise((resolve, reject) => {
            xhr.open('post', url, true);

            if (type && type.isForm == true) {
              xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
            } else {
              xhr.setRequestHeader('Content-Type', 'application/json');
              console.warn("2222222");
            } // 需要加密


            if (isEencrypt && isEencrypt.isEencrypt) {
              data = (_crd && crtptos === void 0 ? (_reportPossibleCrUseOfcrtptos({
                error: Error()
              }), crtptos) : crtptos).encrypt(JSON.stringify(data));
            }

            xhr.onload = function onload() {
              var status = xhr.status !== undefined ? xhr.status : 200;
              console.warn("3333333", status);

              if (status == 200 || status == 204 || status == 0) {
                if (xhr.responseText) {
                  try {
                    var _data;
                    /** 解密处理 看head头部的返回 */


                    var encode = xhr.getResponseHeader("encode");

                    if (encode && encode.toLowerCase() === 'true') {
                      //""
                      _data = JSON.parse((_crd && crtptos === void 0 ? (_reportPossibleCrUseOfcrtptos({
                        error: Error()
                      }), crtptos) : crtptos).decrypt(xhr.responseText));
                    } else {
                      _data = JSON.parse(xhr.responseText);
                    }

                    console.warn("request load::", _data);
                    resolve(_data);
                  } catch (error) {
                    console.warn("request load::", error);
                    reject(error);
                  }
                } else {
                  reject();
                }
              } else {
                if (reject) reject();
                console.warn("netRequesterro", xhr); // observer.post("netRequesterro", xhr);
              }
            };

            xhr.onerror = err => {
              console.warn("4444444", err);
              if (reject) reject(err);
              (_crd && observer === void 0 ? (_reportPossibleCrUseOfobserver({
                error: Error()
              }), observer) : observer).post("netRequesterro", xhr);
            };

            if (type && type.isForm == true) {
              xhr.send(data);
            } else {
              if (!data) data = {};

              if (isEencrypt && isEencrypt.isEencrypt) {
                xhr.send(data);
                console.warn("request send::", data);
              } else {
                console.warn("request stringify send::", data);
                xhr.send(JSON.stringify(data));
              }
            }
          });
        }

        static get(url) {
          if (url === void 0) {
            url = '';
          }

          var xhr = new XMLHttpRequest();
          xhr.withCredentials = true;
          xhr.timeout = 5000;
          return new Promise((resolve, reject) => {
            xhr.open('get', url, true);

            xhr.onload = function onload() {
              var status = xhr.status !== undefined ? xhr.status : 200;

              if (status == 200 || status == 204 || status == 0) {
                if (xhr.responseText) {
                  try {
                    var data = JSON.parse(xhr.responseText);
                    resolve(data);
                  } catch (error) {
                    reject();
                  }
                } else {
                  reject();
                }
              } else {
                reject();
                (_crd && observer === void 0 ? (_reportPossibleCrUseOfobserver({
                  error: Error()
                }), observer) : observer).post("netRequesterro", xhr);
              }
            };

            xhr.onerror = err => {
              console.log("request err:", xhr, "err::", err);
              (_crd && observer === void 0 ? (_reportPossibleCrUseOfobserver({
                error: Error()
              }), observer) : observer).post("netRequesterro", xhr);
            };

            xhr.send();
          });
        }

      });

      _cclegacy._RF.pop();

      _crd = false;
    }
  };
});
//# sourceMappingURL=a622825e0238c4c738f18683ddf1943c905d450f.js.map